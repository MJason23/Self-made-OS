...
int keywindow_on(LAYER *key_to_window, LAYER *layer_window, int cursor_color);
int keywindow_off(LAYER *key_to_window, LAYER *layer_window, int cursor_color, int cursor_x);
...
void HariMain(void)
{
	...
	*/
	...
	LAYER *layer_bg, *layer_mouse, *layer_window, *layer_console[2];
	TIMER *timer;
	FIFO fifo_mutual;									//缓冲区管理，所有中断公用的
	FIFO keycmd_fifo;									
	TASK *task_a, *task_console[2];
	CONSOLE *console;
	int fifobuf[128];											//缓冲区
	int keycmd_buf[32];
	...
	int j, x, y, last_mcursorX = -1, last_mcursorY= -1;			//last_mcursorX这两个变量记录的是鼠标移动之前的坐标
	...
	unsigned char *buf_bg, *buf_window, *buf_console[2];		//屏幕的大背景会在init_screen的时候画出来，这里只需要一个指向它的指针即可
	...
	*((int *) 0x0fe4) = (int) layer_manage;
	...
	key_to_window = layer_window;					//最初的时由task_a那个小窗口接收键盘输入
	
	layer_slide(layer_console[0], 8, 2);
	layer_slide(layer_console[1], 56, 6);
	/*设置好各个图层的高度*/
	layer_switch(layer_bg, 0);						//把背景图层调为最底层，高度为0
	layer_switch(layer_console[0], 1);					//命令行窗口图层调节为第三层，高度为2
	layer_switch(layer_console[1], 2);
	layer_switch(layer_window, 3);					//窗口图层调节为第二层，高度为1
	layer_switch(layer_mouse, 4);					//鼠标图层调为最高层，高度为3
	/*定时器的初始化及其设置*/
	timer = timer_alloc();
		...
		*/
		...
		else
		{
			...
			if(0 == key_to_window->flags)
			{
				...
				cursor_color = keywindow_on(key_to_window, layer_window, cursor_color);
			}
			...
			if((keyboard_offset <= i) && (i <= keyboard_offset + 255))			
			{
				...
				if(('x' == strings[0] || 'X' == strings[0]) && (0 != key_ctrl) && task_console[0]->tss.ss0 != 0)
				{
				...
				if(strings[0] != 0)				
				{
					//如果当前接收键盘输入的窗口为task_a的情况
					if(key_to_window == layer_window)
					{
						//所有可打印的字符都需要打印出来
						if(cursor_x < 144)
						{
				...
				if((keyboard_offset + 0x0e == i))				
				{
					...
					{
						fifo_put(&key_to_window->task->fifo, 8 + keyboard_offset);
					}
				}
				...
				{
					//先让当前的窗口的光标OFF掉
					cursor_color = keywindow_off(key_to_window, layer_window, cursor_color, cursor_x);
				...
				{
					//发送至命令行窗口，task_a窗口就暂时不接受回车键的输入了
					if(key_to_window != layer_window)
					{
						fifo_put(&key_to_window->task->fifo, 10 + keyboard_offset);		//传入10代表按下了回车键
					}
				}
				//左或右Ctrl ON
								...
								if(0 <= x && x < layer->length && 0 <= y && y <= layer->width)
								{
									...
									if(layer->buffer[y * layer->length + x] != layer->color_luc)
									{
										...
										{	
											...
											{
												console = (CONSOLE *) *((int *) 0x0fec);
												console_putstring_toend(console, "Break by mouse!\n");
												io_cli();
												task_console[0]->tss.eax = (int)&(task_console[0]->tss.esp0);
												task_console[0]->tss.eip = (int)asm_end_app;
												io_sti();
...
int keywindow_off(LAYER *key_to_window, LAYER *layer_window, int cursor_color, int cursor_x)
{
	...
	{	
		cursor_color = -1;
		drawRectangle(layer_window, layer_window->length, COL8_FFFFFF, cursor_x, 28, 2, 15);
	}
	else
	{
		//如果是应用窗口的话，它的flags的0x20位设置成1了；应用窗口是不需要显示光标的
