	...
	*/
	...
	else if(edx == 21)
	{
		...
		if(i < 8)
		{
			...
			{
				reg[7] = (int)fhandle;
				fhandle->buffer = (char *)memmanage_alloc_4K(memmanage, finfo->size);
				fhandle->size = finfo->size;
				fhandle->position = 0;
				file_loadfile(finfo->clusterno, finfo->size, fhandle->buffer, task->fat, (char *)(ADR_DISKIMG + 0x003e00));
			}
		}
	}
	//关闭指定文件
...
int cmd_app(CONSOLE *console, int *fat, char *cmdline)
{
	...
	//把命令行中属于命令的那部分跳过去，即空格之前的那部分，空格之后的才能属于文件名
	for(i = 0; 0 != name[i]; i++)
	{
		if(' ' == name[i])
		{
	...
	//如果发现当前的name最后一个字符不是0，那么说明中间遇到了空格提前跳出了循环
	if(0 != name[i])
	{
		k = i;
		for(i = 0; 0 != name[i]; i++)
