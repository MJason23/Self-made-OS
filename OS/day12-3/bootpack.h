...
typedef struct{
	...
	unsigned char data;					//时间到了之后就是把这个数据填充到缓冲区中
}TIMER;

typedef struct{
	unsigned int count, next, using;	//这个count是用来计时的，每一次计时器发来中断的时候count都要++;next代表下一个指定时间
	TIMER *timers_inorder[MAX_TIMER];	//按指定时间远近排列好的timer指针
...
void timer_init(TIMER *timer, FIFO *fifo, unsigned char data);		//定时器初始化
