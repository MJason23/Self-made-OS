...
;如果需要更改画面模式，只需要下面这个数值即可，目前只是觉得800 x  600比较方便而已。
...
;	0x107 : 1280 x 1024 x 8bit彩色		;这个彩色模式QEMU模拟器不能使用
...
;只有支持VBE的BIOS才能支持高分辨率的显卡模式，所以需要进行检查确认
;确认VBE是否存在（在书上P269）
		;我给给ES赋值为0x9000，给DI赋值为0，给AX赋值为0x4f00，再执行“INT 0x10”，如果有VBE的话，AX就会变成0x004f
		;如果AX没有变成这个值，就只能使用320 X 200的画面了
		MOV		AX,0x9000		
		MOV		ES,AX
		MOV		DI,0
		MOV		AX,0x4f00
...
;检查VBE的版本，如果VBE版本不是2.0以上，也不能使用高分辨率
		MOV		AX,[ES:DI+4]
		CMP		AX,0x0200
		JB		scrn320			; if (AX < 0x0200) goto scrn320
		
;取得画面模式信息，计时VBE是2.0以上，也不一定支持所有模式，所以检查一下是否支持0x105画面模式
		MOV		CX,VBEMODE
		MOV		AX,0x4f01
		INT		0x10
		CMP		AX,0x004f		;如果AX不会变为0x004f，说明这个指定的0x105画面模式不能使用
		JNE		scrn320
		
;画面模式信息的确认，确认三项信息,：颜色数是否为8，是否为调色板模式，画面模式号码可否加上0x4000再进行指定
		CMP		BYTE [ES:DI+0x19],8
		JNE		scrn320
		CMP		BYTE [ES:DI+0x1b],4
		JNE		scrn320
		MOV		AX,[ES:DI+0x00]
		AND		AX,0x0080			;模式属性的bit7是0的话，就说明不能加上0x4000在进行确定，只能换回320 X 200模式
		JZ		scrn320	
...
;键盘数据存取
keystatus:				;用BIOS去的键盘上各种LED指示灯的状态
...
; 必须由asmhead来完成的工作至此其实已经全部完毕了，以后就交由bootpack来完成
