...
#define AR_LDT			0x0082				//代表LDT描述符段
...
/*关于文件读写得句柄信息*/
typedef struct{
	char *buffer;			//记录要读取的文件的首地址（已经把文件拷贝到这块儿连续的内存中了）
	int size;
	int position;
}FILEHANDLE;
...
typedef struct{
	...
	FIFO fifo;
	TSS tss;
	SEGMENT_DESCRIPTOR ldt[2];			//每个任务目前只用了LDT表中的两个描述符（用来指向数据段和代码段的）
	struct CONSOLE *console;			//每个任务由一个console启动
	int ds_base, console_stack;			//每个任务所在的数据段需要指定好，程序使用的栈所在的内存也需要记录
	FILEHANDLE *fhandle;
	int *fat;
	char *cmdline;
	unsigned char lanmode, lanbyte1;	//记录当前语言模式（英文模式，中文模式），以及宽字符的第一个字节
}TASK;
/*任务等级*/
typedef struct{
...
typedef struct{
	...
	int view_length, view_width;			//分别代表console的编辑区域的长度和宽度
	TIMER *timer;
}CONSOLE;
...
void display_widefont(unsigned char *vram, int scrnx, int x, int y, char color, char *font);										//在指定位置显示指定颜色的某个字体的函数，这个函数无编码集支持，所以以后很少用了
