...
void console_runcmd(char *cmdline, CONSOLE *console, int *fat, unsigned int mem_total)
{
	...
	//改变命令行字符模式
	else if(0 == strncmp(cmdline, "lanmode ", 8))
	{
		cmd_lanmode(console, cmdline);
	}
	//不是命令但也不是空行
	else if(0 != cmdline[0])
	{
...
//设置命令行窗口语言模式的函数
void cmd_lanmode(CONSOLE *console, char *cmdline)
{	
	TASK *task = task_now();
	unsigned char mode = cmdline[8] - '0';
	if(mode <= 1)
	{
		task->lanmode = mode;
	}
	...
	{
		console_putstring_toend(console, "Mode number error!\n");
	}
	console_newline(console);
	return ;
}
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	*/
	...
	if(edx == 1) 
	{
		console_putchar(console, eax & 0xff, 1);			//因为只需要取得第一字节就可以了
	} 
	//调用显示整个字符串的函数
	else if(edx == 2)
	{
	...
	{
		//ebx是layer的地址，因为分配的layer地址必定是偶数值，所以如果传进来的是奇数，就说明这个layer不需要刷新
		layer = (LAYER *)(ebx & 0xfffffffe);
		hrb_api_drawline(layer, eax, ecx, esi, edi, ebp);
		if(0 == (ebx & 1))
		{
			//这两个if只是为了让起始坐标小于终点坐标，所以如果起始的横纵坐标大了，就需要调换
			if(eax > esi)
			{
				i = eax;
				eax = esi;
	...
	{
		//先找到一个空闲的buffer用来承载文件内容
		for(i = 0; i < 8; i++)
		{
			if(0 == task->fhandle[i].buffer)
			{
				break;
			}
		}
		fhandle = &task->fhandle[i];
		reg[7] = 0;				//这里先设置好返回值为0，如果没有空余的buffer就直接返回0
		//如果i小于8，就说明还有空余的buffer
		if(i < 8)
		{
			...
			if(0 != finfo)
			{
				...
				fhandle->position = 0;
				//用可自行解压缩的方式对应用程序进行加载
				fhandle->buffer = file_load_tekfile(finfo->clusterno, &fhandle->size, task->fat);
			}
		}
	}
	//关闭指定文件
	else if(edx == 22)
	{
		...
		memmanage_free_4K(memmanage, (int)fhandle->buffer, fhandle->size);
		fhandle->buffer = 0;
	}
	//文件定位
	else if(edx == 23)
	{
		...
		{
			fhandle->position = fhandle->size + ebx;
		}
		if(fhandle->position < 0)
		{
			fhandle->position = 0;
		}
		if(fhandle->position > fhandle->size)
		{
			fhandle->position = fhandle->size;
		}
	}
	//获取文件大小
	else if(edx == 24)
	{
		...
		{
			reg[7] = fhandle->position;
		}
		//2代表当前位置从文件尾算起的偏移量
		else if(2 == ecx)
		{
			reg[7] = fhandle->position - fhandle->size;
		}
	}
	//文件读取
	else if(edx == 25)
	{
		...
		for(i = 0; i < ecx; i++)
		{
			...
			//把文件内容传入到用户指定的内存中去
			*((char *) ebx + ds_base + i) = fhandle->buffer[fhandle->position];
			fhandle->position++;
		}
		reg[7] = i;			//返回读入的文件大小
	}
	...
	{
		//ecx指定最多可以存放多少字节
		for(i = 0; 0 != task->cmdline[i] && i < ecx; i++)
		{
			//把整条命令赋值给task的cmdline
			*((char *) ebx + ds_base + i) = task->cmdline[i];
		}
		*((char *) ebx + ds_base + i) = 0;			//没有复制字符串最后的那个结尾0，所以有可能出错，自己手动加上一个0
		reg[7] = i;
	}
	//获取当前语言模式
	else if(edx == 27)
...
int cmd_app(CONSOLE *console, int *fat, char *cmdline)
{
	...
	LAYER *layer;
	char name[20], *img_adr, *data_adr;
	int seg_size, data_size, esp, data_hrb, appsize;
	int i, k;
	//根据命令行空格或者结尾前的部分生成应用程序的文件名
	for (i = 0; i < 13; i++)
	{
		if (cmdline[i] <= ' ')
		{
	...
	if(0 != finfo)
	{
		...
		img_adr = file_load_tekfile(finfo->clusterno, &appsize, fat);
		//如果应用程序是用C语言写的，那么就要在这里把它调整为从HariMain主函数开始运行
		if(appsize >= 36 && 0 == strncmp(img_adr + 4, "Hari", 4) && 0x00 == *img_adr)
		{
			...
			//为这个任务分配一个内存段，将访问权限加上0x60之后，就可以将段设置成为应用程序专用（把1003到2002作为LDT描述符分配用）
			//这里是在task对应的LDT表中分配数据段和代码段的，而不是在GDT中
			set_segmdesc(task->ldt + 0, finfo->size - 1, (int)img_adr, AR_CODE32_ER + 0x60);
			//为应用程序分配的数据段(把2003到3002作为数据段分配用)
			set_segmdesc(task->ldt + 1, seg_size - 1, (int)data_adr, AR_DATA32_RW + 0x60);
			for(i = 0; i < data_size; i++)
			{
				data_adr[esp + i] = img_adr[data_hrb + i];
			}
			//这里的startapp中0 * 8 + 4和1 * 8 + 4都加了4，代表的是这个短号不是GDT段，而是LDT段
			...
			for(i = 0; i < 8; i++)
			{
				if(0 != task->fhandle[i].buffer)
				{
					memmanage_free_4K(memmanage, (int)task->fhandle[i].buffer, task->fhandle[i].size);
					task->fhandle[i].buffer = 0;
				}
			}
		...
		//任务执行完之后就把这块儿内存释放掉
		memmanage_free_4K(memmanage, (int)img_adr, appsize);
		console_newline(console);
		return 1;
	}
	//没找到文件的话直接返回0
...
void console_newline(CONSOLE *console)
{
	...
	console->cursor_x = 8;
	if(1 == task->lanmode && 0 != task->lanbyte1)
	{
		console->cursor_x += 8;
	}
	return ;
...
void console_task(LAYER *layer, unsigned int mem_total)
{
	...
	unsigned char *hzset = (char *) *((int *) 0xfe8);
	char cmdline[30];						//cmdline用来记录在命令行中输入的命令
	FILEHANDLE fhandle[8];					//目前一个console最多只能同时读取8个文件
	CONSOLE console;
	console.layer = layer;
	console.cursor_x = 8;
	console.cursor_y = 28;
	...
	//如果有中文字库，那么console就默认使用中文编码，否则使用内置的英文半角字库
	if(0xff != hzset[4096])
	{
		task->lanmode = 1;
	}
		...
		else
		{
			...
			if(keyboard_offset <= i && i <= keyboard_offset + 255)
			{
				...
				if(8 + keyboard_offset == i)
				{
					...
					//如果当前要删除的位置已经到了提示符所在的位置，但是发现这个字符并不是提示符，那么就可以删除，并且光标移动到
					//上一行的末尾
					else if(console.cursor_x <= 16)
					{
						//如果k不等于0了，说明输入的命令已经换过行了
						if(console.cursor_x <= 8)
						{
							if(console.cursor_y > 28)
							{
								console_putchar(&console, ' ', 0);
								console.cursor_y -= 16;
						...
						else if(k >= 28 && console.cursor_x == 16)
						{
							console_putchar(&console, ' ', 0);
							console.cursor_x = 8;
							console_putchar(&console, ' ', 0);
				...
				else
				{
					...
					{
						//k = console.cursor_x / 8 - 2;
						cmdline[k] = i - keyboard_offset;
						k++;
						console_putchar(&console, i - keyboard_offset, 1);
					}
					//如果到达了console的最右端，就需要换行了
					else if(console.cursor_x > 240)
					{
						console_putchar(&console, ' ', 0);
						console_newline(&console);
						cmdline[k] = i - keyboard_offset;
