...
void HariMain(void)
{
	...
	buf_window = (unsigned char *)memmanage_alloc_4K(memmanage, 160 * 68);						//为窗口图形的内容分配内存
	/*为各个图形的图层内容进行设定*/
	layer_set(layer_bg, buf_bg, binfo->scrnx, binfo->scrny, -1);			
	layer_set(layer_mouse, buf_cursor, 16, 16 ,99);
	layer_set(layer_window, buf_window, 160, 68, -1);
	/*初始化整个桌面背景*/
	...
	/*初始化窗口*/
	create_window(buf_window, 160, 68, "window");	//制作窗口
	displayStrings_CS(buf_window, 160, 24, 28, COL8_000000, "Welcome to");		//向窗口内写入内容
	displayStrings_CS(buf_window, 160, 24, 44, COL8_000000, "Haribote-OS");
	...
	displayStrings_CS(buf_bg, binfo->scrnx, 120, 64, COL8_FFFFFF,strings);			//用字体显示当前内存容量
