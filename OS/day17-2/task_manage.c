...
void console_task(LAYER *layer)
{
	...
	TIMER *timer;
	TASK *task = task_now();
	int i, count = 0,fifobuf[128], cursor_x = 16, cursor_color = COL8_000000;
	char strings[2];
		...
		else
		{
			...
			if(keyboard_offset <= i && i <= keyboard_offset + 255)
			{
				...
				if(8 + keyboard_offset == i)
				{
					...
					{
						cursor_x -= 8;
						displayStrings_atLayer(layer, cursor_x, 28, COL8_FFFFFF, COL8_000000, "  ");
					}
				}
				//如果是一般字符的情况
				else
				{
					if(cursor_x < 240)
					{
						...
						strings[1] = 0;
						displayStrings_atLayer(layer, cursor_x, 28, COL8_FFFFFF, COL8_000000, strings);
						cursor_x += 8;
					}
				}
			}
			//重新显示光标
			drawRectangle(layer->buffer, layer->length, cursor_color, cursor_x, 28, 2, 15);
			layer_part_refresh(layer, cursor_x, 28, cursor_x + 8, 44);
		}
	}
}
