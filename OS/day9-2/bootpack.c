...
void HariMain(void)
{
	...
	memmanage_free(memmanage, 0x00001000, 0x0009e000);
	memmanage_free(memmanage, 0x00400000, memory_total - 0x00400000);
	sprintf(strings, "free memory:%dKB",memmanage_total(memmanage) / 1024);
	displayStrings_CS(binfo->vram, binfo->scrnx, 120, 64, COL8_FFFFFF,strings);
	...
	for(;;)
	{
		...
		else
		{
			if(0 != fifo_status(&keyFIFO))
			{
				...
				drawRectangle(binfo->vram, binfo->scrnx, COL8_008484, 0, 0, 16,16);
