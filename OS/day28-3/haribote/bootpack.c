...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) layer_manage;
	...
	/*载入汉字字符库*/
	hzset = (unsigned char *)memmanage_alloc_4K(memmanage, 16 * 256 + 32 * 94 * 47);//5170);		//共256个半角英文字符，5170个全角中文字符
	...
	if(0 != finfo)
	{
		file_loadfile(finfo->clusterno, finfo->size, hzset, fat, (char *)(ADR_DISKIMG + 0x003e00));
