...
TIMER *timer_alloc(void)
{
	...
	for(i = 0; i < MAX_TIMER; i++)
	{
		if(TIMER_FLAGS_AVAILABLE == timerctl.timer[i].flags)
		{
			...
			timerctl.timer[i].autocancel_flags = 0;			//默认的自动取消标志位不自动取消
...
//取消定时器（这个定时器知识变成alloc状态，不能被再分配出去，只是不再继续使用而已）
int timer_cancel(TIMER *timer)
{
	...
	eflags = io_load_eflags();
	io_cli();
	//只有当这个timer处于使用状态的时候才有必要取消
	if(TIMER_FLAGS_USING == timer->flags)
	{
		//如果要取消的这个定时器是第一个定时器
		if(timer == timerctl.first_timer)
		{
			tmp_timer = timer->next;
			timerctl.first_timer = tmp_timer;
			timerctl.next_timeout = tmp_timer->timeout;
		}
		//如果要取消的这个定时器不是第一个定时器
		else
		{
			tmp_timer = timerctl.first_timer;
			for(;;)
			{
				if(tmp_timer->next == timer)
				{
					break;
				}
				tmp_timer = tmp_timer->next;
			}
			tmp_timer->next = timer->next;
		}
	...
	io_store_eflags(eflags);
	return 0;				//不需要取消的情况返回0
}

//取消某个fifo对应的所有带有自动取消标志的定时器
void timer_cancelall(FIFO *fifo)
{
	...
	for(i = 0; i < MAX_TIMER; i++)
	{
		tmp_timer = &timerctl.timer[i];
		if(0 != tmp_timer->flags && 0 != tmp_timer->autocancel_flags && fifo == tmp_timer->timer_fifo)
		{
			timer_cancel(tmp_timer);
			timer_free(tmp_timer);
		}
	}
	io_store_eflags(eflags);
	return ;
...
void inthandler20(int *esp)
{
	...
	for(;;)
	{
		...
		//只有到达了指定时间的定时器才被触发
		timer->flags = TIMER_FLAGS_ALLOC;
		if(timer != task_timer)		//如果不是用来执行任务切换的那个定时器则正常的向缓冲区中填充数据
		{
