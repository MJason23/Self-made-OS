...
#define TASK_GDT				3				//定义从GDT的几号开始分配给TSS
...
typedef struct{
	...
	TASK *tasks_in_lv[MAX_TASKLEVELS];		/*指向那些在这个等级中运行着的任务*/
}TASKLEVEL;
/*任务管理*/
typedef struct{
	int lv_now;									/*当前处于活动状态的level*/
	char is_lv_change;							/*记录level是否变化了的变量标志*/
	TASKLEVEL task_level[MAX_TASKLEVELS];		
...
typedef struct{
	...
	TASK *task;			//如果需要在缓冲区填充数据的时候唤醒任务，则这里需要设置一个task指针指示唤醒哪个任务
}FIFO;
...
/*timer.c*/
typedef struct{
	unsigned int timeout, flags;		//分别代表指定时间（即现在的时间加上倒计时时间长度）和使用标志
...
/*fifo.c*/
void init_fifo(FIFO *fifo, int size, int *buf, TASK *task);	//初始化缓冲区的函数
