...
void layer_switch(LAYER *layer, int height)
{
	...
	if(old > height)			//如果把图层向下调的情况
	{
		if(height >= 0)			//依旧处于屏幕内，处于显示状态下
		{
			...
			screen_part_refresh(layer_man, layer->x, layer->y, layer->x + layer->length, layer->y + layer->width, height + 1, old);
		}
		...
		screen_part_refresh(layer_man, layer->x, layer->y, layer->x + layer->length, layer->y + layer->width, 0, old - 1);			
	}
	else if(old < height)			//如果把图层向上调的情况
	{
		...
		screen_part_refresh(layer_man, layer->x, layer->y, layer->x + layer->length, layer->y + layer->width, height, height);	
	}
	return ;
}
...
void layer_slide(LAYER *layer, int x0, int y0)
{
	...
	if(layer->height >= 0)			//只要图层位于显示区域（没有隐藏），就可以进行平移
	{
		...
		screen_part_refresh(layer_man, old_x0, old_y0, old_x0 + layer->length, old_y0 + layer->width, 0, layer->height - 1);					//刷新图层原来的位置
		//移动到新位置后，只刷新自己这一图层就行了，因为它不会影响更高图层的显示，刷新高层图层也是浪费时间，做无用功
		screen_part_refresh(layer_man, x0, y0, x0 + layer->length, y0 + layer->width, layer->height, layer->height);						//刷新图层现在所在的位置
	}
}

//判断指定图层是否与指定区域重合(a和b矩形在x轴的投影相交，同时他们在y轴上的投影也相交即可)
int is_layer_overlap(LAYER *layer, int x0, int y0, int x1, int y1)
{
	//先查看两个矩形在横轴上的投影是否有交集，再查看纵轴上的，如果都有，说明两个矩形有交集
	int lx0 = layer->x, lx1 = layer->x + layer->length, ly0 = layer->y, ly1 = layer->y + layer->width;
	int rx0 = x0, rx1 = x1, ry0 = y0, ry1 = y1;
	//如果指定的刷新区域不是从左上角向右下角指定的话，需要调换一下（rx0和ry0记录的一直是比较小的那个横纵坐标）
	if(x0 > x1)
	{
		rx0 = x1;
		rx1 = x0;
	...
	//这是在横轴上的投影有交集的情况(分为线段相交和包含的情况)
	if((lx0 <= rx0 && rx0 <= lx1) || (lx0 <= rx1 && rx1 <= lx1) || (lx0 <= rx0 && rx1 <= lx1) || (rx0 <= lx0 && lx1 <= rx1))
	{
		//这是在纵轴上的投影有交集的情况
		if((ly0 <= ry0 && ry0 <= ly1) || (ly0 <= ry1 && ry1 <= ly1) || (ly0 <= ry0 && ry1 <= ly1) || (ry0 <= ly0 && ly1 <= ry1))
		{
			return 1;		//都相交就说明两矩形相交
		}
	}
	return 0;				//只要有一个条件不满足说明不相交
}

//刷新高度表地图(只有图层高度改变和平移的时候才需要刷新高度地图表)
void layer_refresh_map(LAYER_MANAGE *layer_man, int x0, int y0, int x1, int y1, int height)
{
	int h, width0, length0, x, y, overlap_startX, overlap_startY, overlap_endX, overlap_endY,layerID4, *p;
	...
	for(h = height; h <= layer_man->top; h++)
	{
		...
		layerID = layer - layer_man->layers;//layer->height;				//用图层的高度作为图层ID
		buf = layer->buffer;
		...
		//图层无透明色的情况
		if(-1 == layer->color_luc)
		{
			if(0 == (layer->x & 3) && 0 == (overlap_startX & 3) && 0 == (overlap_endX & 3))
			{
				overlap_endX = (overlap_endX - overlap_startX) / 4;
				layerID4 = layerID | (layerID << 8) | (layerID << 16) | (layerID << 24);
				for(width0 = overlap_startY; width0 < overlap_endY; width0++)
				{
					y = layer->y + width0;
					x = layer->x + overlap_startX;
					p = (int *) &map[y * layer_man->scrnx + x];
					for(length0 = 0; length0 < overlap_endX; length0++)
					{
						p[length0] = layerID4;
					}
				}
...
//屏幕局部刷新,在指定矩形(x0,y0)和(x1,y1)范围内重绘指定的图层（有可能是多个图层，更经常的是所有图层）
void screen_part_refresh(LAYER_MANAGE *layer_man, int x0, int y0, int x1, int y1, int height1, int height2)		
{
	...
	int length1, layerID4, i, end, *map4, *vram4, *buf4;
	...
	for(h = height1; h <= height2; h++)
	{
		layer = layer_man->layers_order[h];
		buf = layer->buffer;
		layerID = layer - layer_man->layers;		//layer->height;
		overlap_startX = x0 - layer->x;
		overlap_startY = y0 - layer->y;
		overlap_endX = x1 - layer->x;
		overlap_endY = y1 - layer->y;
		...
		//图层左上角横坐标是4的倍数的情况，可以4字节4字节的向内存中写入
		if(0 == (layer->x % 4))
		{
			i = (overlap_startX + 3) / 4;
			end = overlap_endX / 4;
			end = end - i;
			layerID4 = layerID | (layerID << 8) | (layerID << 16) | (layerID << 24);
			for(width0 = overlap_startY; width0 < overlap_endY; width0++)
			{
				y = layer->y + width0;
				//1.先把图层最左面那部分不是4的整数倍的坐标部分用单字节写入
				for(length0 = overlap_startX; length0 < overlap_endX && 0 != (length0 % 4); length0++)
				{
					x = layer->x + length0;
					color = buf[width0 * layer->length + length0];
					//这里的判断就判断这个颜色是不是当初设计的要画成透明的颜色设定值，这个值是随便设定的，只要能跟自己判断出来就行，每个图层都可以不一样
				...
				//2.把从坐标为4的整数倍开始的部分用4字节开始写入
				x = layer->x + length0;
				map4 = (int *)&map[y * layer_man->scrnx + x];
				vram4 = (int *)&vram[y * layer_man->scrnx + x];
				buf4 = (int *)&buf[width0 * layer->length + length0];
				for(i = 0; i < end; i++)
				{
					//如果map4[i]的这4个字节没有透明色就应该都是layerID，否则就不跟layerID相同了
					if(map4[i] == layerID4)
					{
						vram4[i] = buf4[i]; 
					}
					//如果有透明色的话，就一个字节一个字节的写入，透明色的话就不写入
					else
					{
						length1 = length0 + i * 4;
						x = layer->x + length1;
						if(map[y * layer_man->scrnx + x + 0] == layerID)
						{
							vram[y * layer_man->scrnx + x + 0] = buf[width0 * layer->length + length1 + 0];
						}
						if(map[y * layer_man->scrnx + x + 1] == layerID)
						{
							vram[y * layer_man->scrnx + x + 1] = buf[width0 * layer->length + length1 + 1];
						}
						if(map[y * layer_man->scrnx + x + 2] == layerID)
						{
							vram[y * layer_man->scrnx + x + 2] = buf[width0 * layer->length + length1 + 2];
						}
						if(map[y * layer_man->scrnx + x + 3] == layerID)
						{
							vram[y * layer_man->scrnx + x + 3] = buf[width0 * layer->length + length1 + 3];
						}
					}
				}
				//3.对剩下的那部分不是4的整数倍的坐标进行单字节写入
				for(length0 += end * 4; length0 < overlap_endX; length0++)
				{
					x = layer->x + length0;
					color = buf[width0 * layer->length + length0];
					if(layerID == map[y * layer_man->scrnx + x])				
...
void layer_part_refresh(LAYER *layer, int length0, int width0, int length1, int width1)
{
	...
	if(layer->height >= 0)
	{
		screen_part_refresh(layer_man, layer->x + length0, layer->y + width0, layer->x + length1, layer->y + width1, layer->height, layer->height);
	}
	return ;
}
