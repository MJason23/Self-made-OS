...
//宽字节版的显示字符
void display_widefont(unsigned char *vram, int scrnx, int x, int y, char color, char *font)
{
	//由于下载的字库稍微有点儿问题，所以得这么写（字库的文字直接显示出来的话，是顺时针旋转90度的字体）
	int i;
	char *position;
	//字体的特定位置所对应的显示内存的地址的计算公式
	position = vram + y * scrnx + x;
	for(i = 0; i < 16; i++)
	{
		if(0 != (font[i] & 0x01))
		{
...
void displayStrings_atLayer(LAYER *layer, int x, int y, int color, int bg_color, char *strings)
{
	...
	if(0 != task->lanmode && 0 != task->lanbyte1)
	{
		displayStrings_CS(layer->buffer, layer->length, x, y, color,strings);
		layer_part_refresh(layer, x - 8, y, x + length * 8, y + 16);
	}
	else
	{
		displayStrings_CS(layer->buffer, layer->length, x, y, color,strings);
