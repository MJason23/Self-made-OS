...
void HariMain(void)
{
	...
	win = api_create_window(winbuf, 336, 261, -1, "invader");
	api_drawrectangle_onwindow(win, 6, 27, 324, 228, 0);
...
void putstr(int win, char *winbuf, int x, int y, int col, unsigned char *s)
{
	...
	api_drawrectangle_onwindow(win + 1, x, y, i * 8, 16, 0);
	q = winbuf + y * 336;
	t[1] = 0;
	for (;;) {
		c = *s;
		...
		if (c != ' ') {
			if ('a' <= c && c <= 'h') {
				...
				t[0] = *s;
				api_putstring_onwindow(win + 1, x, y, col, 1, t);
			}
		}
		s++;
		x += 8;
	}
	api_refreshwindow(win, x0, y, x, y + 16);
	return;
}
